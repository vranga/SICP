...
(define random-init 501)

(define (rand-stream input-message-stream)

	(define (generate-random-number message previous-random-number)
		(cond
			((eq? message 'generate) (rand-update previous-random-number))
			((pair? message)
					(if (eq? (car message) 'reset)
						(begin
							(newline)
							(display "Reset with seed ")
							(display (cadr message))
							(rand-update (cadr message))
						)
						(error "generate-random-number: Invalid Input" (car message))
					)
			)
			(else
				(error "generate-random-number: Invalid Input" message)
			)
		)
	)

	; Input message stream will contain either 'generate or (list 'reset <seed>)
	(define r
		(stream-cons
			random-init
			(stream-map generate-random-number input-message-stream r)
		)
	)
	r
)

; produces a random number in the range 1-1000
; Not the best random number generator but helps us demonstrate how we can solve this
; problem by using streams and without using assignment
(define (rand-update val)
	; using the formula (ax + b) modulo m
	(define a 482)
	(define b 609)
	(define m 1000)
	(modulo (+ (* a val) b) m)
...
> (define L (list 'generate 'generate 'generate 'generate 'generate 'generate (list 'reset 420) 'generate 'generate  'generate  'generate  'generate  'generate  'generate  'generate  'generate  'generate  'generate  'generate  'generate (list 'reset 501) 'generate 'generate  'generate  'generate  'generate 'generate 'generate 'generate))
> (define IS (make-stream-from-list L))
> (define R (rand-stream IS))
> (display-stream R)
