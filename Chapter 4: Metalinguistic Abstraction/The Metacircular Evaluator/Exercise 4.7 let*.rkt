...
; If we have already implemented let (exercise 4.6) and we want to extend the evaluator to
; handle let*, is it sufficient to add a clause to eval whose action is
...
; As can be seen below in the "let*" section, it is sufficient to add a clause to EVAL whose
; action is:
...
;;; EVAL input:
(let* ((x 3) (y (+ x 2)) (z (+ x y 5))) (* x z))
In proc EVAL-let* to execute: (let* ((x 3) (y (+ x 2)) (z (+ x y 5))) (* x z))
In proc EVAL-let to execute: (let ((x 3)) (let ((y (+ x 2))) (let ((z (+ x y 5))) (* x z))))
Type not found: (lambda (x) (let ((y (+ x 2))) (let ((z (+ x y 5))) (* x z))))
In proc EVAL-lambda to execute: (lambda (x) (let ((y (+ x 2))) (let ((z (+ x y 5))) (* x z))))
In proc EVAL-let to execute: (let ((y (+ x 2))) (let ((z (+ x y 5))) (* x z)))
Type not found: (lambda (y) (let ((z (+ x y 5))) (* x z)))
In proc EVAL-lambda to execute: (lambda (y) (let ((z (+ x y 5))) (* x z)))
Type not found: +
In proc lookup-variable-value to execute: #<procedure:exp>
In proc lookup-variable-value to execute: #<procedure:exp>
In proc EVAL-let to execute: (let ((z (+ x y 5))) (* x z))
Type not found: (lambda (z) (* x z))
In proc EVAL-lambda to execute: (lambda (z) (* x z))
Type not found: +
In proc lookup-variable-value to execute: #<procedure:exp>
In proc lookup-variable-value to execute: #<procedure:exp>
In proc lookup-variable-value to execute: #<procedure:exp>
