...
; This solution installs 'and' and 'or' as new special forms for the evaluator by defining
...
;	false
; )
(define (and? expression) (tagged-list? expression 'and))
(define (and-first-predicate expression)
	(if (not (null? (cdr expression)))
		(cadr expression)
		'()
	)
)
(define (and-rest-predicates expression)
	(if (not (null? (cdr expression)))
		(cddr expression)
		'()
	)
...
(define (EVAL-and expression env)
	...
	(if (null? expression)
		...
		true
		(let ((fp (and-first-predicate expression)) (rp (and-rest-predicates expression)))
			(if (not (null? fp))
				(if (true? (EVAL fp env))
					(EVAL-and (make-and rp) env)
					false
				)
				true
			)
...
;	)
; )
(define (or? expression) (tagged-list? expression 'or))
(define (or-first-predicate expression)
	(if (not (null? (cdr expression)))
		(cadr expression)
		'()
	)
)
(define (or-rest-predicates expression)
	(if (not (null? (cdr expression)))
		(cddr expression)
		'()
	)
...
(define (EVAL-or expression env)
	...
	(if (null? expression)
		...
		false
		(let ((fp (or-first-predicate expression)) (rp (or-rest-predicates expression)))
			(if (not (null? fp))
				(if (true? (EVAL fp env))
					true
					(EVAL-or (make-or rp) env)
				)
				false
			)
		)
...
Calling APPLY on: (append '(q w e r t y) '(z x c v b n))
...
Found value (q w e r t y)
In proc lookup-variable-value to lookup: y
Found value (z x c v b n)
In proc EVAL-if to execute: (if (null? x) y (cons (car x) (append (cdr x) y)))
Calling APPLY on: (null? x)
In proc lookup-variable-value to lookup: null?
Found value (primitive #<procedure:null?>)
In proc lookup-variable-value to lookup: x
Found value (w e r t y)
Calling APPLY on: (cons (car x) (append (cdr x) y))
In proc lookup-variable-value to lookup: cons
Found value (primitive #<procedure:cons>)
Calling APPLY on: (car x)
