...
; The <bindings> and <body> are just as in ordinary let, except that <var> is bound within
; <body> to a procedure whose body is <body> and whose parameters are the variables in the
; <bindings>. Thus, one can repeatedly execute the <body> by invoking the procedure named
; <var>. For example, the iterative Fibonacci procedure (section 1.2.2) can be rewritten
...
;;; EVAL input:
(define (square x) (* x x))
In proc EVAL-definition to evaluate: (define (square x) (* x x))
...
;;; EVAL input:
(define (cube x) (* (square x) x))
In proc EVAL-definition to evaluate: (define (cube x) (* (square x) x))
...
Converted (let ((f (square 4))) (+ (cube f) (* 2 f))) to: ((lambda (f) (+ (cube f) (* 2 f))) (square 4))
In proc EVAL-lambda to evaluate: (lambda (f) (+ (cube f) (* 2 f)))
Applying primitive proc: (primitive #<procedure:*>)
Applying primitive proc: (primitive #<procedure:*>)
Applying primitive proc: (primitive #<procedure:*>)
Applying primitive proc: (primitive #<procedure:*>)
Applying primitive proc: (primitive #<procedure:+>)
...
Converted (let ((a 10) (b 20) (c 30) (d 45)) (* a b c d)) to: ((lambda (a b c d) (* a b c d)) 10 20 30 45)
...
In proc EVAL-definition to evaluate: (define (fib n) (let fib-iter ((a 1) (b 0) (count n)) (if (= count 0) b (fib-iter (+ a b) a (- count 1)))))
In proc EVAL-lambda to evaluate: (lambda (n) (let fib-iter ((a 1) (b 0) (count n)) (if (= count 0) b (fib-iter (+ a b) a (- count 1)))))
...
;;; EVAL input:
(fib 0)
In proc EVAL-let to evaluate: (let fib-iter ((a 1) (b 0) (count n)) (if (= count 0) b (fib-iter (+ a b) a (- count 1))))
Converted (let fib-iter ((a 1) (b 0) (count n)) (if (= count 0) b (fib-iter (+ a b) a (- count 1)))) to: (begin (define fib-iter (lambda (a b count) (if (= count 0) b (fib-iter (+ a b) a (- count 1))))) (fib-iter 1 0 n))
In proc EVAL-begin to evaluate: (begin (define fib-iter (lambda (a b count) (if (= count 0) b (fib-iter (+ a b) a (- count 1))))) (fib-iter 1 0 n))
In proc EVAL-definition to evaluate: (define fib-iter (lambda (a b count) (if (= count 0) b (fib-iter (+ a b) a (- count 1)))))
In proc EVAL-lambda to evaluate: (lambda (a b count) (if (= count 0) b (fib-iter (+ a b) a (- count 1))))
In proc EVAL-if to evaluate: (if (= count 0) b (fib-iter (+ a b) a (- count 1)))
Applying primitive proc: (primitive #<procedure:=>)
