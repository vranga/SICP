...
; algorithm given above. Also write a procedure reduce-poly, analogous to add-poly, that checks to
; see if the two polys have the same variable. If so, reduce-poly strips off the variable and
; passes the problem to reduce-terms, then reattaches the variable to the two term lists supplied
; by reduce-terms.
...
; reduce-poly (for polynomial arguments) or reduce-integers (for scheme-number arguments). You can
; now easily make the rational-arithmetic package reduce fractions to lowest terms by having
; make-rat call reduce before combining the given numerator and denominator to form a rational
; number. The system now handles rational expressions in either integers or polynomials. To test
; your program, try the example at the beginning of this extended exercise:
