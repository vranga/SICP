...
; Exercise 2.97.  a. Implement this algorithm as a procedure reduce-terms that takes two term lists n and d
; as arguments and returns a list nn, dd, which are n and d reduced to lowest terms via the algorithm
; given above. Also write a procedure reduce-poly, analogous to add-poly, that checks to see if the two
; polys have the same variable. If so, reduce-poly strips off the variable and passes the problem to
; reduce-terms, then reattaches the variable to the two term lists supplied by reduce-terms.
...
; and define reduce as a generic operation that calls apply-generic to dispatch to either reduce-poly
; (for polynomial arguments) or reduce-integers (for scheme-number arguments). You can now easily make
; the rational-arithmetic package reduce fractions to lowest terms by having make-rat call reduce before
; combining the given numerator and denominator to form a rational number. The system now handles rational
; expressions in either integers or polynomials. To test your program, try the example at the beginning of
...
(newline)
(define p1 (make-polynomial 'x (make-polynomial-sparse-terms (list (list 1 1) (list 0 1)))))
(display "p1 is: ")
(print p1)
(newline)
...
(print p2)
(newline)
(define p3 (make-polynomial 'x (make-polynomial-sparse-terms (list (list 1 1)))))
(display "p3 is: ")
(print p3)
(newline)
(define p4 (make-polynomial 'x (make-polynomial-sparse-terms (list (list 2 1) (list 0 -1)))))
(display "p4 is: ")
(print p4)
(newline)
